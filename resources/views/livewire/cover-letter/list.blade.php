<main class="p-6" x-data>
    <div class="flex justify-between items-center mb-8">
        <div>
            <h4 class="text-slate-900 dark:text-slate-200 text-2xl font-bold tracking-tight">✉️{{ __('List Cover Letter Saya') }}</h4>
            <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">{{ __('Kelola dan lihat semua Cover Letter Anda') }}</p>
        </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700">
                            <div class="grid grid-cols-2 gap-2 text-xs text-gray-600 dark:text-slate-400 mb-4">
                                ...
                                <div>
                                    <span class="font-semibold">💼 {{ __('Posisi') }}:</span> {{ $coverLetter->applied_position ?? '-' }}
                                </div>
                                <div>
                                    <span class="font-semibold">💼 {{ __('Perusahaan') }}:</span> {{ $coverLetter->company_name ?? '-' }}
                                </div>
                            </div>
                        </div>
