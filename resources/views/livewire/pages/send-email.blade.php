<main class="p-6 max-w-7xl mx-auto">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h4 class="text-slate-900 dark:text-slate-200 text-2xl font-bold tracking-tight">{{ __('Kirim Lamaran Baru') }}</h4>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden" style="border-radius: 2rem !important;">
                <div class="p-6 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="font-bold text-slate-900 dark:text-white">{{ __('Tulis Lamaran Baru') }}</h3>
                            <p class="text-xs text-slate-500">{{ __('Menggunakan antrean (Queue) sistem') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 mb-1">{{ __('Nama Anda') }}</label>
                        <input type="text" wire:model.live="name" class="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 text-black dark:border-slate-700 dark:bg-slate-900 dark:text-white" placeholder="{{ __('Nama Lengkap') }}">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 mb-1">{{ __('Posisi Dilamar') }}</label>
                        <input type="text" wire:model.live="position" class="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 text-black dark:border-slate-700 dark:bg-slate-900 dark:text-white" placeholder="{{ __('Posisi Dilamar') }}">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 mb-1">{{ __('Nama Perusahaan') }}</label>
                        <input type="text" wire:model.live="company_name" class="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 text-black dark:border-slate-700 dark:bg-slate-900 dark:text-white" placeholder="{{ __('Nama Perusahaan') }}">
                    </div>

                    <div class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="md:col-span-1">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('Kepada (Recruiter)') }}</label>
                                <input type="email" wire:model="to" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-black dark:border-slate-700 dark:bg-slate-900 dark:text-white focus:ring-2 focus:ring-primary/20 transition-all" placeholder="{{ __('Email Recruiter') }}">
                                @error('to') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                            </div>
                            <div class="md:col-span-1">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('Subjek') }}</label>
                                <input type="text" wire:model="subject" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-black dark:border-slate-700 dark:bg-slate-900 dark:text-white focus:ring-2 focus:ring-primary/20 transition-all" placeholder="{{ __('Subjek Email') }}">
                                @error('subject') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                            </div>
                            <div class="md:col-span-1">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('Waktu Kirim (Opsional)') }}</label>
                                ...
                                <p class="text-[10px] text-slate-400 mt-1 italic">{{ __('Kosongkan untuk kirim sekarang.') }}</p>
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('Isi Email') }}</label>
                ...
                <div class="p-6 bg-indigo-50/50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700 flex justify-end overflow-visible">
                    <button wire:click="send" class="inline-flex items-center justify-center gap-3 px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-indigo-600/20 whitespace-nowrap flex-shrink-0" wire:loading.attr="disabled">
                        <span wire:loading.remove class="flex items-center gap-2 px-5">
                            <i class="ri-send-plane-fill"></i> {{ __('MASUKKAN ANTREAN') }}
                        </span>
                        <span wire:loading class="flex items-center gap-2 px-5">
                            <i class="ri-loader-4-line animate-spin"></i> {{ __('Memproses...') }}
                        </span>
                    </button>
                </div>
            </div>
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 shadow-sm" style="border-radius: 2rem !important;">
                <h3 class="font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2 text-sm uppercase tracking-wide">
                    <i class="ri-upload-cloud-2-line text-emerald-500"></i> {{ __('Upload Manual') }}
                </h3>
                ...
                <div class="relative group">
                    <label for="externalFiles" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-300 dark:border-slate-600 dark:bg-gray-900/50 rounded-2xl cursor-pointer bg-slate-50 dark:bg-slate-900/50 hover:bg-emerald-50/50 dark:hover:bg-emerald-900/10 hover:border-emerald-400 dark:hover:border-emerald-500 transition-all duration-300 active:scale-[0.98]">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p class="mt-2 text-[10px] text-slate-500 dark:text-slate-400 font-medium text-center px-2">{{ __('Klik untuk pilih file') }}</p>
                            <p class="text-[9px] text-slate-400 dark:text-slate-500 mt-1 italic">{{ __('Total Maksimal 3 File (Sistem + Manual)') }}</p>
                        </div>
            ...
            <div class="p-6 text-white shadow-xl border border-indigo-500/30" style="background-color: #5c57a3ff; border-radius: 2rem !important;">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <p class="text-xs font-bold text-white uppercase tracking-widest">{{ __('Sistem Antrean') }}</p>
                        <p class="text-[10px] text-indigo-100">{{ __('Menggunakan Laravel Queue') }}</p>
                    </div>
                </div>
                <p class="text-xs leading-relaxed text-white/90">
                    {{ __('Email tidak dikirim langsung saat Anda klik tombol. Email dimasukkan ke antrean agar aplikasi tetap cepat. Status akan berubah menjadi <b>Terkirim</b> setelah diproses oleh server.') }}
                </p>
            </div>
        </div>
    </div>
