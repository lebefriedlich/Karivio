<main class="p-6">
    ...
    @if ($files->isEmpty())
        <div
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-slate-200">{{ __('Belum ada File') }}</h3>
            <p class="text-gray-600 dark:text-slate-400 mt-2">{{ __('Mulai dengan membuat CV atau Cover Letter Anda') }}</p>
                ...
                <div class="group relative bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden"
                    ...
                    <div class="p-5">
                        ...
                        <div class="flex items-center text-xs text-slate-400 dark:text-slate-500 mb-6">
                            ...
                            {{ __('Terakhir diubah') }}: {{ $file['date']->diffForHumans() }}
                        </div>
