<main class="p-6" x-data>
    <div class="flex justify-between items-center mb-8">
        <div>
            <h4 class="text-slate-900 dark:text-slate-200 text-2xl font-bold tracking-tight">📃{{ __('List CV Saya') }}</h4>
            <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">{{ __('Kelola dan lihat semua CV Anda') }}</p>
        </div>
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('cv.form') }}" class="btn bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-600/20 px-6 py-3 rounded-xl font-bold flex items-center gap-2 transition-all">
                <i class="ri-add-line text-lg"></i> {{ __('Buat CV Baru') }}
            </a>
        </div>
    ...
    @if ($cvs->isEmpty())
        <div class="bg-gray-50 dark:bg-gray-900/50 border-2 border-dashed border-gray-300 dark:border-slate-700 rounded-lg p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-slate-200">{{ __('Belum ada CV') }}</h3>
            <p class="text-gray-600 dark:text-slate-400 mt-2">{{ __('Mulai dengan membuat CV pertama Anda') }}</p>
                    ...
                    <div class="p-6">
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700">
                            <div class="grid grid-cols-2 gap-2 text-xs text-gray-600 dark:text-slate-400 mb-4">
                                <div>
                                    <span class="font-semibold">📍 {{ __('Lokasi') }}:</span> {{ $cv->location ?? '-' }}
                                </div>
                                <div>
                                    <span class="font-semibold">💼 {{ __('Pengalaman') }}:</span> {{ count($cv->work_experiences ?? []) }}
                                </div>
                                <div>
                                    <span class="font-semibold">🎓 {{ __('Pendidikan') }}:</span> {{ count($cv->education ?? []) }}
                                </div>
                                <div>
                                    <span class="font-semibold">🛠️ {{ __('Hard Skill') }}:</span> {{ count($cv->hard_skills ?? ($cv->technical_skills ?? [])) }}
                                </div>
                                <div>
                                    <span class="font-semibold">🌐 {{ __('Bahasa') }}:</span> {{ $cv->language === 'en' ? 'English' : __('Indonesia') }}
                                </div>
                            </div>
